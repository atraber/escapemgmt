...
import {Component, forwardRef, Inject, ElementRef, Input, OnDestroy} from '@angular/core';
...
export class TimeInput implements MatFormFieldControl<number>, OnDestroy, ControlValueAccessor {
  static nextId = 0;
  ...
  get value(): number | null {
    const {value: {hours, minutes, seconds}} = this.parts;
    if (this.isValid(hours.toString(), minutes.toString(), seconds.toString())) {
      let n = parseInt(hours) * 60 * 60 + parseInt(minutes) * 60 + parseInt(seconds);
      return n;
    }
    return null;
  }
  ...
  constructor(fb: FormBuilder, private fm: FocusMonitor, private elRef: ElementRef<HTMLElement>) {
    ...
    this.parts.controls.hours.valueChanges.subscribe(data => {
      if (data.toString().length == 2) {
        let input: any = this.elRef.nativeElement.querySelector('input.minutes')!;
        input.focus();
        input.select();
      }
    });
    ...
    this.parts.controls.minutes.valueChanges.subscribe(data => {
      if (data.toString().length == 2) {
        let input: any = this.elRef.nativeElement.querySelector('input.seconds')!;
        input.focus();
        input.select();
      }
    });
